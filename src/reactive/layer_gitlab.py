...
def install_gitlab():
    ...
    fetch.configure_sources(update=True,
                            ...
                            keys_var='apt_key')
    if version:
        fetch.apt_install('gitlab-ee='.format(
            version), fatal=True)
    else:
...
def waiting():
    hookenv.status_set(
        'blocked',
        'DB and Redis related, waiting for MySQL relation to complete')


@when_all('gitlab.installed',
          'db.available',
          'db.connected',
          'endpoint.redis.available')
@when_any('config.changed',
          'db.changed',
          'endpoint.redis.changed')
def configure_gitlab(reverseproxy, *args):
    hookenv.status_set(
        'maintenance',
        'Configuring GitLab')
    hookenv.log(("Configuring GitLab, then running gitlab-ctl "
...
def remove_mysql():
    hookenv.status_set(
        'maintenance',
        'Cleaning up removed MySQL relation')
...
def remove_redis():
    hookenv.status_set(
        'maintenance',
        'Cleaning up removed Redis relation')
    hookenv.log("Removing config for: {}".format(
        hookenv.remote_unit()))
